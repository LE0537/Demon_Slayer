...
CKeyUI::CKeyUI(const CKeyUI & rhs)
	: CUI(rhs)
{
}
...
void CKeyUI::Tick(_float fTimeDelta)
{
	m_pTransformCom->Set_State(CTransform::STATE_TRANSLATION, XMVectorSet(m_fX - g_iWinSizeX * 0.5f, -m_fY + g_iWinSizeY * 0.5f, 0.f, 1.f));
}

void CKeyUI::Late_Tick(_float fTimeDelta)
{
...
HRESULT CKeyUI::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Components(TEXT("Com_Texture"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_KeyUI"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
...
HRESULT CKeyUI::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
CKeyUI * CKeyUI::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	...
	{
		ERR_MSG(TEXT("Failed to Created : CKeyUI"));
...
CGameObject * CKeyUI::Clone(void * pArg)
{
	...
	{
		ERR_MSG(TEXT("Failed to Cloned : CKeyUI"));
		Safe_Release(pInstance);
	}
