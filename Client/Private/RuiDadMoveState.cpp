...
CRuiDadState * CMoveState::Tick(CRuiDad* pRuiDad, _float fTimeDelta)
{
	...
	else
	{
		if (pRuiDad->Get_QuestStop() == false)
		{
			...
			{
				m_fShakeTime = 0.f;
				dynamic_cast<CCamera_Dynamic*>(pGameInstance->Find_Layer(LEVEL_ADVRUI, TEXT("Layer_Camera"))->Get_LayerFront())->Set_Shake(CCamera_Dynamic::SHAKE_HIT, 0.3f);
				dynamic_cast<CCamera_Dynamic*>(pGameInstance->Find_Layer(LEVEL_ADVRUI, TEXT("Layer_Camera"))->Get_LayerFront())->Blur_VeryLow(pRuiDad->Get_Renderer());
			}
