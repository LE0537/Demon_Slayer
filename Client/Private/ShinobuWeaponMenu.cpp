#include "stdafx.h"
#include "..\Public\ShinobuWeaponMenu.h"
...
CShinobuWeaponMenu::CShinobuWeaponMenu(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CShinobuWeaponMenu::CShinobuWeaponMenu(const CShinobuWeaponMenu & rhs)
	: CGameObject(rhs)
{
}

HRESULT CShinobuWeaponMenu::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CShinobuWeaponMenu::Initialize(void * pArg)
{
	if (nullptr != pArg)
...
void CShinobuWeaponMenu::Tick(_float fTimeDelta)
{
...
void CShinobuWeaponMenu::Late_Tick(_float fTimeDelta)
{


}

HRESULT CShinobuWeaponMenu::Render()
{
...
HRESULT CShinobuWeaponMenu::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CShinobuWeaponMenu::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("ShinobuWeapon(Menu)"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CShinobuWeaponMenu::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
CShinobuWeaponMenu * CShinobuWeaponMenu::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CShinobuWeaponMenu*	pInstance = new CShinobuWeaponMenu(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CShinobuWeaponMenu"));
...
CGameObject * CShinobuWeaponMenu::Clone(void * pArg)
{
	CShinobuWeaponMenu*	pInstance = new CShinobuWeaponMenu(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CShinobuWeaponMenu"));
		Safe_Release(pInstance);
	}
