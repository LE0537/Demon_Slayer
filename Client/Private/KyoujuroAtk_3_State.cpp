...
CKyoujuroState * CAtk_3_State::Late_Tick(CKyoujuro * pKyoujuro, _float fTimeDelta)
{
	...
	else if(m_fMove < 1.f)
	{
		if (!m_bHit)
		{
			...
			if (pMyCollider->Collision(pTargetCollider))
			{
				...
				CSoundMgr::Get_Instance()->PlayEffect(TEXT("Kyojuro_SE_Hit_Attack_2.wav"), g_fEffect);
			}
		}
	}
	...
	if (!m_bEffect)
	{
		...
		pEffectManger->Create_Effect(CEffect_Manager::EFF_RGKATK3, pKyoujuro);
...
void CAtk_3_State::Enter(CKyoujuro * pKyoujuro)
{
	...
	if (iRand == 0)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Kyojuro_Attack3_1.wav"), g_fVoice);
	else if (iRand == 1)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Kyojuro_Attack3_2.wav"), g_fVoice);
	else if (iRand == 2)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Kyojuro_Attack3_3.wav"), g_fVoice);
...
CKyoujuroState * CAtk_3_State::CommandCheck(CKyoujuro * pKyoujuro)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
