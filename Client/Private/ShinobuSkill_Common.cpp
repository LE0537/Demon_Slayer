...
CShinobuState * CSkill_CommonState::Late_Tick(CShinobu* pShinobu, _float fTimeDelta)
{
		...
		if (m_fHitTime > 0.1f && m_iHit < 5)
		{
			...
			if (pMyCollider->Collision(pTargetCollider))
			{
				...
				else if (pShinobu->Get_BattleTarget()->Get_GodMode() == false)
				{
					...
					RELEASE_INSTANCE(CGameInstance);
					m_pTarget->Set_Hp(-15 * pShinobu->Get_PlayerInfo().fPowerUp);
	...
	if (!m_bEffect)
	{
		...
		pEffectManger->Create_Effect(CEffect_Manager::EFF_SHINOBU_SKL_COM_1, pShinobu);
		pEffectManger->Create_Effect(CEffect_Manager::EFF_SHINOBU_SKL_COM_2, pShinobu);
