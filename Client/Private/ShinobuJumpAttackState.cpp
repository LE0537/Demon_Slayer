...
CShinobuState * CJumpAttackState::Late_Tick(CShinobu* pShinobu, _float fTimeDelta)
{
	if (m_eStateType == CShinobuState::TYPE_START)
	{
		...
		if (!m_bHit)
		{
			...
			if (pMyCollider->Collision(pTargetCollider))
			{
				...
				CSoundMgr::Get_Instance()->PlayEffect(TEXT("Shinobu_SE_Hit_JumpAttack.wav"), g_fEffect);
	...
	{
		CEffect_Manager* pEffectManger = GET_INSTANCE(CEffect_Manager);
		pEffectManger->Create_Effect(CEffect_Manager::EFF_SHINOBU_ATK_JUMP_COM_MAIN, pShinobu);
		RELEASE_INSTANCE(CEffect_Manager);
		m_bEffect = true;
	}
...
void CJumpAttackState::Enter(CShinobu* pShinobu)
{
	...
	case Client::CShinobuState::TYPE_START:
		...
		CSoundMgr::Get_Instance()->PlayEffect(TEXT("Shinobu_SE_JumpAttack.wav"), g_fEffect);
		break;
	case Client::CShinobuState::TYPE_LOOP:
