...
CTanjiroState * CAtk_2_KaguraState::Late_Tick(CTanjiro * pTanjiro, _float fTimeDelta)
{
	...
	if (m_fMove < 0.3f)
	{
		...
		if (m_iHit < 1)
		{
			...
			if (pMyCollider->Collision(pTargetCollider))
			{
				...
				{
					CSoundMgr::Get_Instance()->PlayEffect(TEXT("Hinokami_SE_Hit_Attack_1.wav"), fEFFECT);
					_int iDest = rand() % 5;
					CEffect_Manager* pEffectManger = GET_INSTANCE(CEffect_Manager);
					switch (iDest)
					{
	...
	if (!m_bEffect)
	{
		...
		pEffectManger->Create_Effect(CEffect_Manager::EFF_HINOATT2_1, pTanjiro);
		pEffectManger->Create_Effect(CEffect_Manager::EFF_HINOATT2_2, pTanjiro);
...
void CAtk_2_KaguraState::Enter(CTanjiro * pTanjiro)
{
	...
	CSoundMgr::Get_Instance()->PlayEffect(TEXT("Hinokami_SE_Attack_1.wav"), fEFFECT);
}
