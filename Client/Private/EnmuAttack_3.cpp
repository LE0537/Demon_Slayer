...
CEnmuState * CEnmuAttack3::Late_Tick(CEnmu* pEnmu, _float fTimeDelta)
{
	...
	if (!m_bEffect)
	{
		...
		pEffectManger->Create_Effect(CEffect_Manager::EFF_ENMU_ATK3_1, pEnmu);
		pEffectManger->Create_Effect(CEffect_Manager::EFF_ENMU_ATK3_2, pEnmu);
		pEffectManger->Create_Effect(CEffect_Manager::EFF_ENMU_ATK3_3, pEnmu);
...
void CEnmuAttack3::Enter(CEnmu* pEnmu)
{
	...
	CSoundMgr::Get_Instance()->PlayEffect(TEXT("Enmu_SE_Attack_2.wav"), g_fEffect);

	_uint iRand = rand() % 2;

	if (iRand == 0)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Enmu_Attack2_0.wav"), g_fVoice);
	else if (iRand == 1)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Enmu_Attack2_1.wav"), g_fVoice);
}
