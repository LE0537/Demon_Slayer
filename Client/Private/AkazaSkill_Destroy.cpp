			...
			if (m_iHit < 1)
			{
				...
				if (pMyCollider->Collision(pTargetCollider))
				{
					...
					else
					{
						...
						RELEASE_INSTANCE(CGameInstance);
						m_pTarget->Set_Hp(_int(-20 * pAkaza->Get_PlayerInfo().fPowerUp));
			...
			if (!m_bHit)
			{
				...
				if (pMyCollider->Collision(pTargetCollider))
				{
					...
					else
					{
						...
						m_pTarget->Player_UpperDown(CCharacters::HIT_TYPE::HIT_UPPER, 25.f, 20.f, 1.f);
...
void CSkill_DestoryState::Enter(CAkaza* pAkaza)
{
	...
	case Client::CAkazaState::TYPE_START:
		...
		CSoundMgr::Get_Instance()->PlayEffect(TEXT("Akaza_SE_Destroy.wav"), fEFFECT);
		break;
	case Client::CAkazaState::TYPE_END:
