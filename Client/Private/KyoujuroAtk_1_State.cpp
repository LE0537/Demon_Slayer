	...
	if (m_fMove < 0.3f)
	{
		...
		if (m_fMove > 0.1f && m_iHit == 0)
		{
			...
			if (pMyCollider->Collision(pTargetCollider))
			{
				...
				CSoundMgr::Get_Instance()->PlayEffect(TEXT("Kyojuro_SE_Hit_Attack_0.wav"), g_fEffect);
			}
...
void CAtk_1_State::Enter(CKyoujuro * pKyoujuro)
{
	...
	if (iRand == 0)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Kyojuro_Attack1_1.wav"), g_fVoice);
	else if (iRand == 1)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Kyojuro_Attack1_2.wav"), g_fVoice);
	else if (iRand == 2)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Kyojuro_Attack1_3.wav"), g_fVoice);

		CSoundMgr::Get_Instance()->PlayEffect(TEXT("Nezuko_SE_Attack_0.wav"), g_fEffect);
}
