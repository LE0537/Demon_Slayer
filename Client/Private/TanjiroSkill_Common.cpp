...
CTanjiroState * CSkill_CommonState::Late_Tick(CTanjiro * pTanjiro, _float fTimeDelta)
{
		...
		if (m_eStateType != Client::CTanjiroState::TYPE_START)
		{
			...
			if (m_fHitTime > 0.07f && iHit < 5 && pTanjiro->Get_BattleTarget()->Get_GodMode() == false)
			{
				...
				if (pMyCollider->Collision(pTargetCollider))
				{
					...
					{
						CGameInstance*		pGameInstance2 = GET_INSTANCE(CGameInstance);
						dynamic_cast<CCamera_Dynamic*>(pGameInstance2->Find_Layer(g_iLevel, TEXT("Layer_Camera"))->Get_LayerFront())->Set_Shake(CCamera_Dynamic::SHAKE_DOWN, 0.4f);
						RELEASE_INSTANCE(CGameInstance);
	...
	if (!m_bEffect)
	{
		...
		pEffectManger->Create_Effect(CEffect_Manager::EFF_WATER8_1_GROUND, pTanjiro);
