...
CTanjiroState * CAtk_3_State::Late_Tick(CTanjiro * pTanjiro, _float fTimeDelta)
{
	...
	else if (m_fMove < 0.7f)
	{
		if (!m_bHit)
		{
			...
			if (pMyCollider->Collision(pTargetCollider))
			{
				...
				{
					CSoundMgr::Get_Instance()->PlayEffect(TEXT("Tanjiro_SE_Hit_Attack_20.wav"), g_fEffect);
					_int iDest = rand() % 5;
					CEffect_Manager* pEffectManger = GET_INSTANCE(CEffect_Manager);
					switch (iDest)
					{
	...
	if (!m_bEffect)
	{
		...
		pEffectManger->Create_Effect(CEffect_Manager::EFF_TANATTACK3, pTanjiro);
		if (g_iLevel != LEVEL_BATTLEENMU)
			pEffectManger->Create_Effect(CEffect_Manager::EFF_TANATTACK3_GROUND, pTanjiro);
...
void CAtk_3_State::Enter(CTanjiro * pTanjiro)
{
	...
	if (iRand == 0)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Tanjiro_Attack3_1.wav"), g_fVoice);
	else if (iRand == 1)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Tanjiro_Attack3_2.wav"), g_fVoice);
	else if (iRand == 2)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Tanjiro_Attack3_3.wav"), g_fVoice);

	CSoundMgr::Get_Instance()->PlayEffect(TEXT("Tanjiro_SE_Attack_3.wav"), g_fEffect);
}
...
CTanjiroState * CAtk_3_State::CommandCheck(CTanjiro * pTanjiro)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
