...
void CBigBird::Tick(_float fTimeDelta)
{
	...
	//if (pGameInstance->Key_Down(DIK_F7))	// 추가 -> 저장하기
	//{
	//	CData_Manager* pData_Manager = GET_INSTANCE(CData_Manager);
	//	char cName[MAX_PATH];
	//	ZeroMemory(cName, sizeof(char) * MAX_PATH);
	//	pData_Manager->TCtoC(TEXT("BigBird"), cName);
	//	pData_Manager->Conv_Bin_Model(m_pModelCom, cName, CData_Manager::DATA_ANIM);
	//	ERR_MSG(TEXT("Save_Bin_BigBird"));
...
HRESULT CBigBird::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CBigBird::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("BigBird"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CBigBird::Take_Damage(_float _fPow, _bool _bJumpHit)
{
}

void CBigBird::Get_GuardHit(_int eType)
{
}
...
CBigBird * CBigBird::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	...
	{
		ERR_MSG(TEXT("Failed to Created : CBigBird"));
...
{
	CGameObject*	pInstance = new CBigBird(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CBigBird"));
		Safe_Release(pInstance);
	}
