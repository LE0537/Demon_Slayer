	...
	else if (m_fMove < 0.45f && m_fMove >= 0.3f)
	{
		if (!m_bHit)
		{
			...
			if (pMyCollider->Collision(pTargetCollider))
			{
				...
				CSoundMgr::Get_Instance()->PlayEffect(TEXT("Kyojuro_SE_Hit_Attack_1.wav"), g_fEffect);
			}
		}
	}
...
void CAtk_2_State::Enter(CKyoujuro * pKyoujuro)
{
	...
	if (iRand == 0)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Kyojuro_Attack2_1.wav"), g_fVoice);
	else if (iRand == 1)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Kyojuro_Attack2_2.wav"), g_fVoice);
	else if (iRand == 2)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Kyojuro_Attack2_3.wav"), g_fVoice);
