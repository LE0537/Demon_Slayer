#include "stdafx.h"
#include "..\Public\KyoujuroSheath.h"
...
CKyoujuroSheath::CKyoujuroSheath(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CKyoujuroSheath::CKyoujuroSheath(const CKyoujuroSheath & rhs)
	: CGameObject(rhs)
{
}

HRESULT CKyoujuroSheath::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CKyoujuroSheath::Initialize(void * pArg)
{
	if (nullptr != pArg)
...
void CKyoujuroSheath::Late_Tick(_float fTimeDelta)
{
...
HRESULT CKyoujuroSheath::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CKyoujuroSheath::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("KyoujuroSheath"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CKyoujuroSheath::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
CKyoujuroSheath * CKyoujuroSheath::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CKyoujuroSheath*	pInstance = new CKyoujuroSheath(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CKyoujuroSheath"));
...
CGameObject * CKyoujuroSheath::Clone(void * pArg)
{
	CKyoujuroSheath*	pInstance = new CKyoujuroSheath(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CKyoujuroSheath"));
		Safe_Release(pInstance);
	}
