...
CAkazaState * CJumpMoveAttackState::Late_Tick(CAkaza* pAkaza, _float fTimeDelta)
{
	...
	if (!m_bEffect && m_eStateType == TYPE_LOOP)
	{
		...
		pEffectManger->Create_Effect(CEffect_Manager::EFF_AKZATK_JUMPMOVE_NONFOLLOW, pAkaza);
		pEffectManger->Create_Effect(CEffect_Manager::EFF_AKZATK_JUMPMOVE_FOLLOW, pAkaza);
	...
	else if (!m_bEffect && m_eStateType == TYPE_END)
	{
		...
		pEffectManger->Create_Effect(CEffect_Manager::EFF_AKZATK_JUMPMOVE_END, pAkaza);
...
void CJumpMoveAttackState::Enter(CAkaza* pAkaza)
{
	...
	case Client::CAkazaState::TYPE_START:
		...
		if(pAkaza->Get_BattleTarget()->Get_TargetState() == STATE_HIT)
			CSoundMgr::Get_Instance()->PlayEffect(TEXT("Akaza_SE_Hit_JumpMoveAttack.wav"), g_fEffect);
		else
			CSoundMgr::Get_Instance()->PlayEffect(TEXT("Akaza_SE_JumpMoveAttack.wav"), g_fEffect);
		break;
	case Client::CAkazaState::TYPE_LOOP:
