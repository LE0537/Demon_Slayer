...
CShinobuState * CAtk_2_State::Late_Tick(CShinobu* pShinobu, _float fTimeDelta)
{
	...
	if (m_fMove < 0.4f)
	{
		...
		if (m_fMove > 0.15f && m_iHit < 2 && m_fDelay > 0.15f)
		{
			...
			if (pMyCollider->Collision(pTargetCollider))
			{
				...
				CSoundMgr::Get_Instance()->PlayEffect(TEXT("Shinobu_SE_Hit_Attack_1.wav"), g_fEffect);
	...
	if (!m_bEffect)
	{
		...
		pEffectManger->Create_Effect(CEffect_Manager::EFF_SHINOBU_ATK_2_1, pShinobu);
		pEffectManger->Create_Effect(CEffect_Manager::EFF_SHINOBU_ATK_2_2, pShinobu);
...
void CAtk_2_State::Enter(CShinobu* pShinobu)
{
	...
	if (iRand == 0)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Shinobu_Attack2_1.wav"), g_fVoice);
	else if (iRand == 1)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Shinobu_Attack2_2.wav"), g_fVoice);
	else if (iRand == 2)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Shinobu_Attack2_3.wav"), g_fVoice);

	CSoundMgr::Get_Instance()->PlayEffect(TEXT("Shinobu_SE_Attack_1.wav"), g_fEffect);
}
...
CShinobuState * CAtk_2_State::CommandCheck(CShinobu * pShinobu)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
