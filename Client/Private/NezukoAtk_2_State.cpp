...
CNezukoState * CAtk_2_State::HandleInput(CNezuko* pNezuko)
{
...
CNezukoState * CAtk_2_State::Tick(CNezuko* pNezuko, _float fTimeDelta)
{
	...
	if (pNezuko->Get_Model()->Get_End(CNezuko::ANIM_ATTACK_2))
	{
		pNezuko->Get_Model()->Set_End(CNezuko::ANIM_ATTACK_2);
...
CNezukoState * CAtk_2_State::Late_Tick(CNezuko* pNezuko, _float fTimeDelta)
{
	...
	if (!m_bEffect)
	{
		...
		pEffectManger->Create_Effect(CEffect_Manager::EFF_NEZUKO_ATK2, pNezuko);
...
void CAtk_2_State::Enter(CNezuko* pNezuko)
{
	...
	if (iRand == 0)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Nezuko_Attack1_1.wav"), g_fVoice);
	else if (iRand == 1)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Nezuko_Attack1_2.wav"), g_fVoice);
	else if (iRand == 2)
		CSoundMgr::Get_Instance()->PlayVoice(TEXT("Nezuko_Attack1_3.wav"), g_fVoice);

	if (pNezuko->Get_BattleTarget()->Get_TargetState() == STATE_HIT)
		CSoundMgr::Get_Instance()->PlayEffect(TEXT("Nezuko_SE_Hit_Attack_1.wav"), g_fEffect);
	else
		CSoundMgr::Get_Instance()->PlayEffect(TEXT("Nezuko_SE_Attack_1.wav"), g_fEffect);
}
...
CNezukoState * CAtk_2_State::CommandCheck(CNezuko * pNezuko)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
