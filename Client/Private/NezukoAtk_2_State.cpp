...
CNezukoState * CAtk_2_State::Late_Tick(CNezuko* pNezuko, _float fTimeDelta)
{
	...
	if (m_fMove < 0.5f)
	{
		pNezuko->Get_Transform()->Go_Straight(fTimeDelta * 1.3f, pNezuko->Get_NavigationCom());
		...
		{
			_float fSpeed = pNezuko->Get_Transform()->Get_TransformDesc().fSpeedPerSec * fTimeDelta * 1.3f;
