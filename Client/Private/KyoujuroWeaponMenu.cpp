#include "stdafx.h"
#include "..\Public\KyoujuroWeaponMenu.h"
...
CKyoujuroWeaponMenu::CKyoujuroWeaponMenu(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CKyoujuroWeaponMenu::CKyoujuroWeaponMenu(const CKyoujuroWeaponMenu & rhs)
	: CGameObject(rhs)
{
}

HRESULT CKyoujuroWeaponMenu::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CKyoujuroWeaponMenu::Initialize(void * pArg)
{
	if (nullptr != pArg)
...
void CKyoujuroWeaponMenu::Tick(_float fTimeDelta)
{
	...
	if (m_bMenu)
	{
		matCollBox.r[3] += XMVector3Normalize(vLook) * 50.f;
		XMStoreFloat4x4(&m_CombinedWorldMatrix, matCollBox);
	}

}

void CKyoujuroWeaponMenu::Late_Tick(_float fTimeDelta)
{


}

HRESULT CKyoujuroWeaponMenu::Render()
{
...
HRESULT CKyoujuroWeaponMenu::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CKyoujuroWeaponMenu::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("KyoujuroWeapon(Menu)"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CKyoujuroWeaponMenu::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
CKyoujuroWeaponMenu * CKyoujuroWeaponMenu::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CKyoujuroWeaponMenu*	pInstance = new CKyoujuroWeaponMenu(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CKyoujuroWeaponMenu"));
...
CGameObject * CKyoujuroWeaponMenu::Clone(void * pArg)
{
	CKyoujuroWeaponMenu*	pInstance = new CKyoujuroWeaponMenu(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CKyoujuroWeaponMenu"));
		Safe_Release(pInstance);
	}
