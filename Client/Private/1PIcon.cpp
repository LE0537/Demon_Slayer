...
C1PIcon::C1PIcon(const C1PIcon & rhs)
	: CUI(rhs)
{
}

HRESULT C1PIcon::Initialize_Prototype()
{
	return S_OK;
}

HRESULT C1PIcon::Initialize(void * pArg)
{
...
HRESULT C1PIcon::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Components(TEXT("Com_Texture"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_1P_SelIcon"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
...
HRESULT C1PIcon::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
C1PIcon * C1PIcon::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	C1PIcon*	pInstance = new C1PIcon(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : C1PIcon"));
...
CGameObject * C1PIcon::Clone(void * pArg)
{
	C1PIcon*	pInstance = new C1PIcon(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : C1PIcon"));
		Safe_Release(pInstance);
	}
