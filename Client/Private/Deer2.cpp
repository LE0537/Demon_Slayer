...
CDeer2::CDeer2(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CCharacters(pDevice, pContext)
{
}

CDeer2::CDeer2(const CDeer2 & rhs)
	: CCharacters(rhs)
{
}

HRESULT CDeer2::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CDeer2::Initialize(void * pArg)
{
...
void CDeer2::Tick(_float fTimeDelta)
{
...
void CDeer2::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CDeer2::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CDeer2::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CDeer2::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Deer2"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CDeer2::Take_Damage(_float _fPow, _bool _bJumpHit)
{
}

void CDeer2::Get_GuardHit(_int eType)
{
}
...
CDeer2 * CDeer2::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	...
	{
		ERR_MSG(TEXT("Failed to Created : CDeer2"));
...
CGameObject * CDeer2::Clone(void * pArg)
{
	CGameObject*	pInstance = new CDeer2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CDeer2"));
		Safe_Release(pInstance);
	}
