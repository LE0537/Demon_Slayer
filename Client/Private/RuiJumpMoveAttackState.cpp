...
void CJumpMoveAttackState::Enter(CRui* pRui)
{
	...
	case Client::CRuiState::TYPE_START:
		...
		if (iRand == 0)
			CSoundMgr::Get_Instance()->PlayVoice(TEXT("Rui_JumpMoveAttack1.wav"), g_fVoice);
		else if (iRand == 1)
			CSoundMgr::Get_Instance()->PlayVoice(TEXT("Rui_JumpMoveAttack2.wav"), g_fVoice);

		if (pRui->Get_BattleTarget()->Get_TargetState() == STATE_HIT)
			CSoundMgr::Get_Instance()->PlayEffect(TEXT("Rui_SE_Hit_JumpMoveAttack.wav"), g_fEffect);
		else
			CSoundMgr::Get_Instance()->PlayEffect(TEXT("Rui_SE_JumpMoveAttack.wav"), g_fEffect);
		break;
	case Client::CRuiState::TYPE_LOOP:
