...
CQuiestGuideBase::CQuiestGuideBase(const CQuiestGuideBase & rhs)
	: CUI(rhs)
{
}

HRESULT CQuiestGuideBase::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CQuiestGuideBase::Initialize(void * pArg)
{
...
HRESULT CQuiestGuideBase::Render()
{
...
HRESULT CQuiestGuideBase::Ready_Components()
{
	/* For.Com_Renderer */
...
HRESULT CQuiestGuideBase::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
CQuiestGuideBase * CQuiestGuideBase::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CQuiestGuideBase*	pInstance = new CQuiestGuideBase(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CQuiestGuideBase"));
...
CGameObject * CQuiestGuideBase::Clone(void * pArg)
{
	CQuiestGuideBase*	pInstance = new CQuiestGuideBase(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CQuiestGuideBase"));
		Safe_Release(pInstance);
	}
