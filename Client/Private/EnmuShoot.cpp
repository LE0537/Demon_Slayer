#include "stdafx.h"
#include "..\Public\EnmuShoot.h"

#include "GameInstance.h"
#include "Layer.h"
#include "Effect_Manager.h"
CEnmuShoot::CEnmuShoot(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CCollBox(pDevice, pContext)
{
}

CEnmuShoot::CEnmuShoot(const CEnmuShoot & rhs)
	: CCollBox(rhs)
{
}

HRESULT CEnmuShoot::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CEnmuShoot::Initialize(void * pArg)
{
	if (FAILED(Ready_Components()))
		return E_FAIL;

	memcpy(&m_ShootInfo, pArg, sizeof(ENMUSHOOTINFO));
...
void CEnmuShoot::Tick(_float fTimeDelta)
{
...
void CEnmuShoot::Late_Tick(_float fTimeDelta)
{
...
HRESULT CEnmuShoot::Render_ShadowDepth()
{
	return S_OK;
}

HRESULT CEnmuShoot::Ready_Components()
{
	/* For.Com_Renderer */
...
CEnmuShoot * CEnmuShoot::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CEnmuShoot*	pInstance = new CEnmuShoot(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CEnmuShoot"));
...
CGameObject * CEnmuShoot::Clone(void * pArg)
{
	CGameObject*	pInstance = new CEnmuShoot(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CEnmuShoot"));
		Safe_Release(pInstance);
	}
