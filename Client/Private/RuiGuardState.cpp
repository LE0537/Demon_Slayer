...
CRuiState * CGuardState::HandleInput(CRui* pRui)
{
	...
	{
	case 1:
		if (pGameInstance->Key_Pressing(DIK_O) && m_eStateType != STATE_TYPE::TYPE_END && pRui->Get_PlayerInfo().fGuardTime <= 0.f)
		{
		...
		break;
	case 2:
		if (pGameInstance->Key_Pressing(DIK_C) && m_eStateType != STATE_TYPE::TYPE_END && pRui->Get_PlayerInfo().fGuardTime <= 0.f)
		{
			pRui->Set_bGuard(true);
