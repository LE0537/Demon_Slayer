...
CShinobu::CShinobu(const CShinobu & rhs)
	: CCharacters(rhs)
{
}

HRESULT CShinobu::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CShinobu::Initialize(void * pArg)
{
...
void CShinobu::Tick(_float fTimeDelta)
{
	if (!m_bChange)
	{
		if (m_bBattleStart)
...
HRESULT CShinobu::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CShinobu::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CShinobu::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Shinobu"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CShinobu::Ready_Parts()
{
	...
	CShinobuWeapon::WEAPONDESC		WeaponDesc;
	...
	m_pWeapon = pGameInstance->Clone_GameObject(TEXT("Prototype_GameObject_ShinobuWeapon"), &WeaponDesc);
...
HRESULT CShinobu::Ready_Parts2()
{
	...
	m_pSheath = pGameInstance->Clone_GameObject(TEXT("Prototype_GameObject_ShinobuSheath"), &WeaponDesc);
...
{
	CShinobu*	pInstance = new CShinobu(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CShinobu"));
...
CGameObject * CShinobu::Clone(void * pArg)
{
	CGameObject*	pInstance = new CShinobu(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CShinobu"));
		Safe_Release(pInstance);
	}
