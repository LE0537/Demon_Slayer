...
CTanjiroState * CAtk_1_KaguraState::Late_Tick(CTanjiro * pTanjiro, _float fTimeDelta)
{
	...
	if (m_fMove < 0.3f)
	{
		...
		if (m_fMove > 0.1f && !m_bHit)
		{
			...
			if (pMyCollider->Collision(pTargetCollider))
			{
				...
				{
					CSoundMgr::Get_Instance()->PlayEffect(TEXT("Hinokami_SE_Hit_Attack_0.wav"), fEFFECT);
					_int iDest = rand() % 5;
					CEffect_Manager* pEffectManger = GET_INSTANCE(CEffect_Manager);
					switch (iDest)
					{
	...
	if (!m_bEffect)
	{
		...
		pEffectManger->Create_Effect(CEffect_Manager::EFF_HINOATT1, pTanjiro);
...
void CAtk_1_KaguraState::Enter(CTanjiro * pTanjiro)
{
	...
	CSoundMgr::Get_Instance()->PlayEffect(TEXT("Hinokami_SE_Attack_0.wav"), fEFFECT);
}
...
CTanjiroState * CAtk_1_KaguraState::CommandCheck(CTanjiro * pTanjiro)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();

	m_fDuration = pTanjiro->Get_Model()->Get_Duration_Index(CTanjiro::ANIM_KAGURA_ATTACK_1);
	m_fCurrentDuration = pTanjiro->Get_Model()->Get_CurrentTime_Index(CTanjiro::ANIM_KAGURA_ATTACK_1);
