...
void CVIBuffer_Point_Instance::Set_Pos(vector<_float4> _vecPos, vector<_float3> _vecScale)
{
	D3D11_MAPPED_SUBRESOURCE		MappedSubResource;
	ZeroMemory(&MappedSubResource, sizeof MappedSubResource);
	...
	for (_uint i = 0; i < m_iNumInstance; ++i)
	{
		((VTXMATRIX*)MappedSubResource.pData)[i].vRight = _float4(_vecScale[i].x, 0.f, 0.f, 0.f);
		((VTXMATRIX*)MappedSubResource.pData)[i].vUp = _float4(0.f, _vecScale[i].y, 0.f, 0.f);
		((VTXMATRIX*)MappedSubResource.pData)[i].vLook = _float4(0.f, 0.f, _vecScale[i].z, 0.f);
		((VTXMATRIX*)MappedSubResource.pData)[i].vPosition = _vecPos[i];
	}
